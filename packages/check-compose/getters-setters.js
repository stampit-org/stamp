...
module.exports = compose => {
  ...
  test("getter and setter", nest => {
    nest.test("should merge into stamp descriptor", assert => {
      ...
      const stamp = getsetProps.reduce(
        (stamp, propName) => stamp.compose({ [propName]: createGS() }),
        compose()
      );
      ...
      getsetProps.forEach(propName => {
        ...
        assert.equal(
          typeof info.get,
          "function",
          "should have getter in " + propName
        );
        assert.equal(
          typeof info.set,
          "function",
          "should have setter in " + propName
        );
      });
    ...
    nest.test(
      "should be okay to overwrite with different getters and setters",
      assert => {
        const createGS = () => ({
          get info() {
            return `getter${this._info}`;
        ...
        stamp = getsetProps.reduce(
          (stamp, propName) => stamp.compose({ [propName]: createGS2() }),
          stamp
        );
        ...
        assert.equal(
          typeof info.get,
          "function",
          "should have getter on object instance"
        );
        assert.equal(
          typeof info.set,
          "function",
          "should have setter on object instance"
        );
        assert.equal(instance.info, "2", "getter should be overtwitten");
        instance.info = "3";
        assert.equal(instance._info, "33", "setter should be overtwitten");

        const info2 = Object.getOwnPropertyDescriptor(
          instance.__proto__,
          "info"
        );
        assert.equal(
          typeof info2.get,
          "function",
          "should have getter on object prototype"
        );
        assert.equal(
          typeof info2.set,
          "function",
          "should have setter on object prototype"
        );
        assert.equal(
          instance.__proto__.info,
          "2",
          "getter should be overtwitten"
        );
        instance.__proto__.info = "3";
        assert.equal(
          instance.__proto__._info,
          "33",
          "setter should be overtwitten"
        );
        ...
        const info4 = Object.getOwnPropertyDescriptor(
        ...
        assert.equal(
          stamp.compose.configuration.info,
          "2",
          "getter should be overtwitten"
        );
        stamp.compose.configuration.info = "3";
        assert.equal(
          stamp.compose.configuration._info,
          "33",
          "setter should be overtwitten"
        );
    ...
    nest.test("in methods", assert => {
      ...
      assert.equal(
        instance.info,
        "getter_initial_PREFIX-TEXT",
        "Getter should be copied"
      );
      assert.equal(
        instance._info,
        "_initial_PREFIX-TEXT",
        "Setter should be copied"
      );
