...
module.exports = compose => {
  test("composer arguments", t => {
    ...
    t.ok(
      _.includes(stamp.compose.composers, composer),
  ...
  test("composers should be deduped", t => {
    ...
    t.equal(
      _.uniq(composers).length,
      composers.length,
      "should dedupe composers"
    );
  ...
  test("stamp.compose({ composers() }) passes full composables array", t => {
    ...
    function composer(ref) {
      const composables = ref.composables;
      ...
      if (run === 2) {
        t.ok(
          _.includes(composers, composer),
          "inheriting stamp should still pass our composer"
        );
        t.ok(
          _.includes(composables, stamp),
          "composables must contain stamp itself"
        );
        t.ok(
          _.includes(composables, stamp2),
          "composables must contain second stamp too"
        );
      }
    }
    var stamp = compose({
