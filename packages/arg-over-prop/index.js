 ...
 * TODO
 */
const ArgOverProp = compose_1.default({
    staticProperties: {
        argOverProp(...args) {
            let propNames = [];
            let defaultProps;
            args.forEach((arg) => {
                if (is_1.isString(arg)) {
                    propNames.push(arg);
                }
                else if (is_1.isArray(arg)) {
                    propNames = [...propNames, ...arg.filter(is_1.isString)];
                }
                else if (is_1.isObject(arg)) {
                    defaultProps = core_1.assign(defaultProps || {}, arg);
